...
export function AuthProvider({ children }) {
  ...
  async function getPublicBuilds(limit = 6) {
    try {
      const { data } = await supabase.from('builds').select('*').eq('is_public', true).order('created_at', { ascending: false }).limit(limit)
      return { data: data || [] }
    } catch(e) { return { data: [] } }
  }
  ...
  async function getBuildById(id) {
    try {
      const { data, error } = await supabase.from('builds').select('*').eq('id', id).eq('is_public', true).single()
      return { data, error }
  ...
  return (
    <AuthContext.Provider value={{ user, profile, loading, signUp, signIn, signOut, saveBuild, getMyBuilds, deleteBuild, getPublicBuilds, getBuildById, updateProfile }}>
      {children}
