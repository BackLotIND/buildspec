...
const FEED_CAT = {
  ...
  new_release:    { l:'New Release',    c:'#10B981' },
}
...
// ── Skeleton ──────────────────────────────────────────────────
function SkeletonBlock({ w='100%', h=12, r=6, mb=0 }) {
  return (
    <div style={{
      width:w, height:h, borderRadius:r,
      background:`linear-gradient(90deg,${C.s2} 25%,${C.bdr} 50%,${C.s2} 75%)`,
...
export function FeedSkeleton() {
  return (
    <>
      {[0,1,2,3].map(i => (
        <div key={i} style={{ background:C.s1, borderRadius:12, border:`1px solid ${C.bdr}`, padding:'0.9rem 1rem', opacity: 1-i*0.15 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10 }}>
            <SkeletonBlock w={80} h={18} r={20}/>
            <SkeletonBlock w={30} h={10} r={4}/>
          </div>
          <SkeletonBlock h={16} r={6} mb={8}/>
          <SkeletonBlock w='80%' h={11} r={6} mb={12}/>
          <SkeletonBlock w='60%' h={10} r={6}/>
        </div>
      ))}
    </>
  )
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:10 }}>
          {comments.map(c => (
            <div key={c.id} style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
              <div style={{ width:24, height:24, borderRadius:'50%', flexShrink:0, background:`linear-gradient(135deg,${C.acc},${C.g})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.6rem', fontWeight:800, color:'#fff', fontFamily:fm }}>
      ...
      {canComment ? (
        <div style={{ display:'flex', gap:8, alignItems:'flex-end' }}>
          <textarea
            ...
            rows={1}
            style={{ flex:1, padding:'7px 10px', borderRadius:8, border:`1px solid ${draft?C.acc+'80':C.bdr}`, background:C.s2, color:C.t, fontSize:'0.7rem', fontFamily:fs, outline:'none', resize:'none', lineHeight:1.4, transition:'border-color 0.15s' }}
          />
          <button onClick={submit} disabled={!draft.trim()||submitting} style={{ padding:'7px 14px', borderRadius:8, border:'none', background:draft.trim()?C.acc:C.s2, color:draft.trim()?'#fff':C.td, fontSize:'0.68rem', fontWeight:700, cursor:draft.trim()?'pointer':'default', fontFamily:fs, flexShrink:0, transition:'background 0.15s,color 0.15s', opacity:submitting?0.6:1 }}>
            {submitting?'…':'Post'}
          </button>
        </div>
      ) : user?.id ? (
        <div style={{ fontSize:'0.62rem', color:C.td, fontStyle:'italic' }}>Upgrade to Member or Pro to comment.</div>
      ) : (
        <div style={{ fontSize:'0.62rem', color:C.td, fontStyle:'italic' }}>Sign in as a member to comment.</div>
      )}
...
function Timestamp({ d }) {
  return <span style={{ fontSize:'0.52rem', color:C.td, fontFamily:fm, flexShrink:0, marginLeft:8 }}>{timeAgo(d)}</span>
}

function CommentBtn({ expanded, onToggle }) {
  return (
    <button onClick={e => { e.stopPropagation(); onToggle() }} style={{ background:'none', border:`1px solid ${expanded?C.acc+'60':C.bdr}`, borderRadius:20, padding:'3px 10px', color:expanded?C.acc:C.td, fontSize:'0.58rem', cursor:'pointer', fontFamily:fs, display:'flex', alignItems:'center', gap:4, transition:'border-color 0.15s,color 0.15s', WebkitTapHighlightColor:'transparent' }}>
      💬 {expanded?'Hide':'Reply'}
    </button>
  )
}
...
function StatPill({ icon, value, color }) {
  return <span style={{ fontSize:'0.58rem', color:color||C.td, display:'flex', alignItems:'center', gap:3 }}>{icon} {value}</span>
}
...
export default function FeedCard({ item, i, user, profile, supabase, makes, platforms, liked, onLike }) {
  ...
  const borderColor = { news:C.bdr, thread:C.bdr, bounty:'#F9731625', review:`${C.g}25` }[item._type] || C.bdr
  const anim = { animation:`fadeUp 0.3s ease-out ${i*0.03}s both` }
  const card = { background:C.s1, borderRadius:12, border:`1px solid ${borderColor}`, padding:'0.9rem 1rem', ...anim }

  // ── NEWS ──
  if (item._type === 'news') {
    const cat = FEED_CAT[item.category] || { l: item.category||'News', c: C.tm }
    return (
      <div style={card}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <button onClick={() => { if(liked) return; onLike(item.id) }} style={{ background:'none', border:`1px solid ${liked?C.acc:C.bdr}`, borderRadius:20, padding:'3px 10px', color:liked?C.acc:C.td, fontSize:'0.58rem', cursor:liked?'default':'pointer', fontFamily:fs, display:'flex', alignItems:'center', gap:4 }}>
            {liked?'❤️':'🤍'} {item.like_count||0}
          </button>
          <CommentBtn expanded={expanded} onToggle={()=>setExpanded(v=>!v)}/>
          <a href="/news" style={{ fontSize:'0.58rem', color:C.td, textDecoration:'none', marginLeft:'auto', opacity:0.7 }}>Read →</a>
        </div>
    ...
    return (
      <div style={card}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          ...
          <StatPill icon="❤️" value={item.like_count||0} color={item.like_count>0?C.acc:C.td}/>
          <StatPill icon="💬" value={item.reply_count||0}/>
          <CommentBtn expanded={expanded} onToggle={()=>setExpanded(v=>!v)}/>
        </div>
        {expanded && <CommentSection item={item} user={user} profile={profile} supabase={supabase}/>}
      </div>
    ...
    return (
      <div style={card}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
          <StatPill icon="💬" value={`${item.response_count||0} responses`}/>
          <CommentBtn expanded={expanded} onToggle={()=>setExpanded(v=>!v)}/>
          <a href="/bounties" style={{ fontSize:'0.58rem', color:'#F97316', textDecoration:'none', fontWeight:600, marginLeft:'auto' }}>View →</a>
        </div>
        {expanded && <CommentSection item={item} user={user} profile={profile} supabase={supabase}/>}
      </div>
    )
  ...
  if (item._type === 'review') {
    ...
    return (
      <div style={card}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
          {item.would_recommend && <StatPill icon="✓" value="Recommended" color={C.g}/>}
          <CommentBtn expanded={expanded} onToggle={()=>setExpanded(v=>!v)}/>
        </div>
        {expanded && <CommentSection item={item} user={user} profile={profile} supabase={supabase}/>}
      </div>
